	...
	public class MyInputField : Selectable, IUpdateSelectedHandler, IEventSystemHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler, ISubmitHandler, ICanvasElement, ILayoutElement
	{
		...
		private IEnumerator MouseDragOutsideRect(PointerEventData eventData)
		{
			while (m_UpdateDrag && m_DragPositionOutOfBounds)
			{
				...
				UpdateLabel();
				float time = multiLine ? 0.1f : 0.05f;
		...
		private void MoveDown(bool shift, bool goToLastChar)
		{
			...
			int num4 = multiLine ? LineDownCharacterPosition(caretSelectPositionInternal, goToLastChar) : text.Length;
			if (shift)
			{
				caretSelectPositionInternal = num4;
			}
		...
		private void Backspace()
		{
			if (!m_ReadOnly)
			{
				...
				else if (caretPositionInternal > 0)
				{
					...
					int num3 = caretSelectPositionInternal = (caretPositionInternal -= 1);
					SendOnValueChangedAndUpdateLabel();
				}
			}
		}
		...
		private void Insert(char c)
		{
			if (!m_ReadOnly)
			{
				...
				if (characterLimit <= 0 || this.text.Length < characterLimit)
				{
					...
					int num3 = caretSelectPositionInternal = (caretPositionInternal += text.Length);
					SendOnValueChanged();
				}
			}
		}
