	...
	public class BinTree : InWindow, IMatchFinder, IInWindowStream
	{
		...
		public uint GetMatches(uint[] distances)
		{
			...
			if (HASH_ARRAY)
			{
				...
				num10 = (uint)((num9 ^ (int)(CRC.Table[_bufferBase[num4 + 3]] << 5)) & (int)_hashMask);
			}
			else
			{
			...
			uint num18 = (_cyclicBufferPos << 1) + 1;
			uint num19 = _cyclicBufferPos << 1;
			uint val;
			...
			uint num22 = _cutValue;
			while (true)
			{
				if (num11 <= num3 || num22-- == 0)
				{
					_son[num18] = (_son[num19] = 0u);
					break;
				}
				uint num24 = _pos - num11;
				uint num25 = ((num24 <= _cyclicBufferPos) ? (_cyclicBufferPos - num24) : (_cyclicBufferPos - num24 + _cyclicBufferSize)) << 1;
				...
				if (_bufferBase[num26 + num27] == _bufferBase[num4 + num27])
				{
					...
					if (num5 < num27)
					{
						...
						if (num27 == num)
						{
							_son[num19] = _son[num25];
							_son[num18] = _son[num25 + 1];
							break;
						}
					}
				}
		...
		public void Skip(uint num)
		{
			do
			{
				...
				if (HASH_ARRAY)
				{
					...
					num9 = (uint)((num7 ^ (int)(CRC.Table[_bufferBase[num4 + 3]] << 5)) & (int)_hashMask);
				}
				else
				{
				...
				while (true)
				{
					...
					if (_bufferBase[num17 + num18] == _bufferBase[num4 + num18])
					{
						...
						if (num18 == num2)
						{
							_son[num12] = _son[num16];
							_son[num11] = _son[num16 + 1];
							break;
						}
					}
					if (_bufferBase[num17 + num18] < _bufferBase[num4 + num18])
